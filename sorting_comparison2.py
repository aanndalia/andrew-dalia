...
def run_trials(stop, increment):
    ...
    return x, y_merge, y_insertion, y_selection, y_quick
    
def plot(x, y_merge, y_insertion, y_selection, y_quick):
    pyplot.figure(1)
    pyplot.subplot(211)
    ...
    pyplot.axis([0, max(x), 0, max(max(y_merge), max(y_insertion), max(y_selection), max(y_quick))])
    #pyplot.xscale('log')
    #pyplot.yscale('log')
    pyplot.xlabel('Items')
    pyplot.ylabel('Time')
...
def main():
    ...
    print "quick sorted arr2", quick_sort(arr2)
    
    x, y_m, y_i, y_s, y_q = run_trials(7500, 500)
    plot(x, y_m, y_i, y_s, y_q)
